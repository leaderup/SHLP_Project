...
def user_agent(device_type):
    """
    根据设备类型返回随机的浏览器头部，通常用在爬虫使用
    :param device_type: 设备类型;android,ios,pc
    :return: 随机返回设备对应的浏览器头部user_agent
    """
    android_headers = [
    ...
    if device_type == 'android':
        return random.choice(android_headers)
    elif device_type == 'ios':
        return random.choice(ios_headers)
    elif device_type == 'pc':
