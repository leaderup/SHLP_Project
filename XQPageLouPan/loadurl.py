...
def resetUncatchStatus():
    # 重置数据状态
    sqlstring = u"UPDATE lp_links SET `detail_status`=0 WHERE `detail_status`=1"
...
def urlput(urlqueue, datadir, log=logging.getLogger("urlput")):
    ...
    while 1:
        if empty_execution == 10:
            sqlstring = "SELECT count(*) as rownums FROM %s WHERE `detail_status`=%s" % (lp_link, status)
        ...
        if qsize >= max_size:
            time.sleep(2)
            continue
        sqlstring = "SELECT * FROM %s WHERE `detail_status`=%s LIMIT 0, %s" % (lp_link, status, max_size)
        # sqlstring = "SELECT * FROM %s WHERE `detail_status`=%s and `id`=25985 LIMIT 0, %s" % (lp_link, status, max_size)
        rinit = conn.query(sqlstring)
        if len(rinit):
            empty_execution = 0
            for row in rinit:
                if row['detail_url']:
                    sqlstring = "UPDATE %s SET `detail_status`=1 WHERE id='%s'" % (lp_link, row["id"])
                    rowcount = conn.execute_rowcount(sqlstring)
                    if rowcount == 1:
                        row['url'] = row['detail_url']
                        storepath = "_".join([str(row['cid']), "loupanDetails", "".join(lazy_pinyin(row['name']))])
                        row['urlpath'] = os.path.join(datadir, "%s.html" % storepath)
                        urlqueue.put(row)
                else:
                    sqlstring = "UPDATE %s SET `detail_status`=6 WHERE id='%s'" % (lp_link, row["id"])
                    conn.execute(sqlstring)
        else:
            empty_execution += 1
