...
def analysisPage(info, dataDir):
    ...
    # 小区名称
    lp_name_tag = soup.select("div.sftitle div.lpbt h1 a")[0]
    lp_name = lp_name_tag.text.strip()
    first_page_info['title'] = lp_name
    # 小区别名 (无)
    # 标签
    lp_labels = []
    lp_label_tags = soup.select("div.sftitle div.lpname.fl div.lpicon a")
...
if __name__ == "__main__":
    ...
    f = open(r"D:\PythonProgram\SHLouPan\data\Test\xiangtihuayuan022.html")
    content = f.read()
    f.close()
    soup = BeautifulSoup(content, "html5lib", from_encoding="utf-8")
    info = {}
    info['url'] = u"http://xiangtihuayuan022.fang.com/"
    info['soup'] = soup
