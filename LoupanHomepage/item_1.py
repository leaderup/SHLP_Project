...
def analysisPage(info, dataDir):
    ...
    # 小区别名
    lp_alias = ""
    lp_name_tag = soup.select("div.sftitle div.lpname div.lpbt span")
    if lp_name_tag:
        lp_alias = lp_name_tag[0].text.strip().replace(u"别名：", "")
    first_page_info['alias'] = lp_alias
    # 标签
    lp_labels = []
    lp_label_tags = soup.select("div.sftitle div.lpname div.lpicon div a")
...
if __name__ == "__main__":
    ...
    f = open(r"D:\PythonProgram\SHLouPan\data\Test\guangyaochengtbh.html")
    content = f.read()
    f.close()
    soup = BeautifulSoup(content, "html5lib", from_encoding="utf-8")
    info = {}
    info['url'] = u"http://guangyaochengtbh.fang.com/"
    info['soup'] = soup
    info['name'] = u"朗诗未来街区"
    info['id'] = 1 # test
