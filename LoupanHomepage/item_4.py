...
def analysisPage(info, dataDir):
    ...
    lp_path_tag = soup.select("div.br_left ul.tf")[0]
    ...
    # 小区名称
    lp_name_tag = soup.select("div.sftitle div.lpname div.lpbt h1 a")[0]
    lp_name = lp_name_tag.text.strip()
    first_page_info['title'] = lp_name
    # 小区别名
    lp_alias = ""
    lp_name_tag = soup.select("div.lpname div.lpbt span.h1_label")
    if lp_name_tag:
        lp_alias = lp_name_tag[0].text.strip().replace(u"别名：", "")
    first_page_info['alias'] = lp_alias
    # 标签
    lp_labels = []
    lp_label_tags = soup.select("div.lpicon div a")
    ...
    # 首页详情信息
    first_details = []
    first_detail_tags = soup.select("div.firstbox div.firstright div.information div.information_li div.inf_left p")
    for first_detail_tag in first_detail_tags:
        first_detail = blank_to_one.sub(" ", first_detail_tag.text.strip())
        first_detail = first_detail.replace(u"\xa0", "").replace(u'\u2002', "")
    ...
    lpxq_link = ""
    lpxc_link = ""
    orginalNaviBox = soup.find(id="orginalNaviBox")
    lp_link_tags = orginalNaviBox.select("a")
...
if __name__ == "__main__":
    ...
    f = open(r"D:\PythonProgram\SHLouPan\data\Test\weilaijiequls.html")
    content = f.read()
    f.close()
    soup = BeautifulSoup(content, "html5lib", from_encoding="utf-8")
    info = {}
    info['url'] = u"http://weilaijiequls021.fang.com/"
    info['soup'] = soup
    info['name'] = u"朗诗未来街区"
    info['id'] = 1 # test
