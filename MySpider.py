...
def add_cookie(cookies_src, cookies_des):
    """
    requests库请求返回cookie添加到cookies_des字典中
    :param cookies_src: 新增cookies(requests.cookies.RequestsCookieJar)
    :param cookies_des: 生成目标cookies(dict)
    :return:cookies_des
    """
    for cookie in cookies_src:
        try:
            key = ((str(cookie).split('=')[0]).split(' '))[1]
            value = ((str(cookie).split('=')[1]).split(' '))[0]
            cookies_des[key] = value
        except Exception, ex:
            print ex
    return cookies_des
